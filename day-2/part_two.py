...
def compute_safe_report_count(reports):
    ...
    for report in reports:
        # We keep trying to evaluate if report is "safe", first by
        # using whole report, then by removing one element per iteration
        # until we've tried removing each element.
        for i in range(-1, len(report)):
            report_copy = [level for j, level in enumerate(report) if i == -1 or i != j]
            if validate_condition_one(report_copy) and validate_condition_two(report_copy):
                safe_count += 1
                break
    return safe_count
...
def test_sample():
    assert compute_safe_report_count(get_input("sample_input.txt")) == 4
